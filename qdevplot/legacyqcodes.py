...
def plot_legacy_qcodes_linescoop(
    filepath, ivallist=None, x_label=None, y_label=None, cb_label=None, invertaxes=False
):
    ...
    ax1, cb = df_to_pcolor(
        df, dims=dim, ax=ax1, invertaxes=invertaxes
    )  # qp.df_to_scatter(df, ax=ax1)
    xmin = df.iloc[:, 0].min()
    xmax = df.iloc[:, 0].max()
    legend_list = []
...
def read_legacy_qcodes(filepath, invertaxes: bool = False, **kwargs):
    ...
    df.rename(columns={df.columns[0]: df.columns[0].split('"')[1::2][0]}, inplace=True)
