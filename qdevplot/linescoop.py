...
class LineScoop:
    ...
    def plot_line_scoop_from_df_line(
        ...
        self.ax_line.quiver(
            self.df_plot[col_names[0]].tolist(), self.df_plot[col_names[2]].tolist(),
            dist_to_point * np.sin(-np.arctan(a_norm)) * range_of_column(self.df_plot[col_names[0]]),
            dist_to_point * np.cos(np.arctan(a_norm)) * range_of_column(self.df_plot[col_names[2]]),
            color=["r"],
            label='reference',
            angles='xy', 
            scale_units='xy',
        ...
        self.ax_line.legend(loc='best', fontsize=7,
...
def get_scoop_line(df, a, b, delta, xlim=(-np.inf, np.inf), ylim=(-np.inf, np.inf), scale=None):
    ...
    if scale == None:
        a_norm = a * range_of_column(df[col_names[0]]) / range_of_column(df[col_names[1]])
    
    
    df["distsign"] = (df[X] * a + b - df[Y]) / range_of_column(df[col_names[1]]) * a_norm / (
        a_norm**2 + 1
    ) ** 0.5
